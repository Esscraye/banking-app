...
describe("TransactionsPage", () => {
  ...
  describe("Data Loading", () => {
    ...
    it("should handle API errors gracefully", async () => {
      (accountsService.getAccounts as jest.Mock).mockRejectedValue(
        new Error("API Error"),
      );
      (transactionsService.getTransactions as jest.Mock).mockRejectedValue(
        new Error("API Error"),
      );
  ...
  describe("New Transaction Form", () => {
    ...
    it("should hide transaction form when Cancel is clicked", async () => {
      ...
      expect(
        screen.queryByText("Create New Transaction"),
    ...
    it("should populate account dropdown with active accounts", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByDisplayValue("Select an account"),
        ).toBeInTheDocument();
      });
    });
    ...
    it("should handle transaction creation errors", async () => {
      (transactionsService.createTransaction as jest.Mock).mockRejectedValue(
  ...
  describe("Transfer Form", () => {
    it("should show transfer form when Transfer button is clicked", async () => {
      ...
      expect(
        screen.getByDisplayValue("Select source account"),
      ).toBeInTheDocument();
      expect(
        screen.getByDisplayValue("Select destination account"),
      ).toBeInTheDocument();
    });
    ...
    it("should submit transfer form with valid data", async () => {
      ...
      // Fill form
      const fromAccountSelect = screen.getByDisplayValue(
        "Select source account",
      );
      const toAccountSelect = screen.getByDisplayValue(
    ...
    it("should handle transfer errors", async () => {
      (transactionsService.transfer as jest.Mock).mockRejectedValue(
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Failed to process transfer"),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Transaction Display", () => {
    ...
    it("should display account information for each transaction", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getAllByText("checking - 1234567890")[0],
        ).toBeInTheDocument();
